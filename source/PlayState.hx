...
class PlayState extends MusicBeatState
{
	...
	private var blackAlone:FlxSprite;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'friends' | 'expurgation':	
				defaultCamZoom = 0.8;
				
				var bgGardenNew:BGSprite = new BGSprite('spinel/friends/sky', -500, -100, 0.1, 0.1);
				add(bgGardenNew);
								
				var place:BGSprite = new BGSprite('spinel/friends/place', -300, -5, 1.0, 1.0);
				place.setGraphicSize(Std.int(place.width * 0.95));
				place.updateHitbox();
				add(place);
				
				var pillars:BGSprite = new BGSprite('spinel/friends/pillars', -443, -1, 1.7, 1.7);
				pillars.setGraphicSize(Std.int(place.width * 1.4));
				add(pillars);
				
			case 'flowers' | 'friendship':
				curStage = 'deadGarden';
				defaultCamZoom = 0.8;
				
				var bgGardenDead:BGSprite = new BGSprite('spinel/flowers/sky2', -500, -100, 0.1, 0.1);
				add(bgGardenDead);

				var place:BGSprite = new BGSprite('spinel/flowers/place_but_old', -300, -5, 1.0, 1.0);
				place.setGraphicSize(Std.int(place.width * 0.95));
				place.updateHitbox();
				add(place);
				
			case 'betrayed' | 'postbetrayed':
			    curStage = 'inyectorGarden';
				defaultCamZoom = 0.4;
				//defaultCamZoom = 0.4;
				
				//I TAKE 2 DAYS FOR CODE THIS SHIT.. PLEASE... I JUST WANT TO SLEEP
				
				var bg:BGSprite = new BGSprite('spinel/forgive/skyblue', -1890, -2160, 1.0, 1.0);
				bg.setGraphicSize(Std.int(bg.width * 1.50));
				bg.updateHitbox();
				add(bg);
				
				var bg:BGSprite = new BGSprite('spinel/forgive/skyshader', -1890, -2160, 1.0, 1.0);
				bg.setGraphicSize(Std.int(bg.width * 1.50));
				bg.updateHitbox();
				add(bg);
				
				var glow:BGSprite = new BGSprite('spinel/forgive/glow', -1890, -2160, 1.0, 1.0);
				glow.setGraphicSize(Std.int(glow.width * 1.50));
				glow.updateHitbox();
				add(glow);	
								
				var place:BGSprite = new BGSprite('spinel/forgive/place_but_injector', -1890, -2160, 1.0, 1.0);
				place.setGraphicSize(Std.int(place.width * 1.50));
				place.updateHitbox();
				add(place);	

            case 'friends-beta':
                var poopyBG:FlxSprite = new FlxSprite(-400, -100).loadGraphic(Paths.image('spinel/betaGarden/poopyGardenBG','heart'));
				add(poopyBG);			
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'friends':				
				    inCutscene = true;
                    dialogueIntro(dialogue, 'blammed_inst_bg');	
					
				case 'flowers':				
				    inCutscene = true;
                    dialogueIntro(dialogue, 'otherfriends_inst_bg');
	...
	{			
		if (curSong == "postbetrayed")
		{
            new FlxTimer().start(0.1, function(tmr:FlxTimer)
			{
			    FlxG.sound.playMusic(Paths.music('spinelMenuLoop', 'heart'));
	...
	function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'White On':
				...
				{
				    dadGroup.remove(dad);
					dad = new Character(DAD_X, DAD_Y, 'spinel-happy');
					dadGroup.add(dad);
					iconP2.changeIcon(dad.healthIcon);
            ...
            case 'Spinel/Pico Attack':
                if(boyfriend.animation.getByName('missAttack') != null) {
				    boyfriend.playAnim('missAttack', true);
				    boyfriend.specialAnim = true;
				}
	...
	function finishSong():Void
	{
		...
		var lowercaseSong:String = SONG.song.toLowerCase();
		var file:String = Paths.txt(lowercaseSong + '/' + lowercaseSong + 'DialogueEnd');
		if (OpenFlAssets.exists(file)) {
			dialogue = CoolUtil.coolTextFile(file);
		}
	}
                    ...
                    if(!boyfriend.stunned)
					{
						if(!endingSong)
						{
							...
							if(boyfriend.animation.getByName('missShoot') != null) {
								boyfriend.playAnim('missShoot', true);
								boyfriend.specialAnim = true;
							}
							
							if(dad.animation.getByName('miss') != null) {
